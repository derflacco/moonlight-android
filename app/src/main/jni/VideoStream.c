...
static spsc_ring_t g_rx_ring;
static volatile int g_rx_stopping = 0;
static PLT_THREAD rxBatchThread;
...
static void VideoRxBatchThreadProc(void* ctx) {
    rx_batch_cfg_t cfg;
    cfg.fd = rtpSocket;
    cfg.stopping = &g_rx_stopping;
    cfg.ring = &g_rx_ring;
    cfg.max_batch = 16;
    cfg.max_udp = 2048;
    cfg.rcvbuf_bytes = 8 * 1024 * 1024;
    cfg.micro_backoff_ns = 200000;
    rx_batch_loop(&cfg);
}
...
void stopVideoStream(void) {
    ...
    PltJoinThread(&receiveThread);
        g_rx_stopping = 1;
    PltJoinThread(&rxBatchThread);
    spsc_ring_free(&g_rx_ring);
if ((VideoCallbacks.capabilities & (CAPABILITY_DIRECT_SUBMIT | CAPABILITY_PULL_RENDERER)) == 0) {
...
int startVideoStream(void* rendererContext, int drFlags) {
    ...
    /* Start RX batch thread (recvmmsg) */
    spsc_ring_init(&g_rx_ring, 1024);
    g_rx_stopping = 0;
    PltCreateThread("VideoRxBatch", VideoRxBatchThreadProc, NULL, &rxBatchThread);
